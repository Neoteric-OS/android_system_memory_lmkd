...
static void __mp_event_psi(enum event_source source, union psi_event_data data,
    ...
    if (events &&
       (!poll_params->poll_handler || data.level >= poll_params->poll_handler->data)) {
...
no_kill:
    ...
    if (events || killing || reclaim == DIRECT_RECLAIM || reclaim == DIRECT_RECLAIM_THROTTLE) {
        ...
        } else if (!poll_params->poll_handler || data.level >= poll_params->poll_handler->data) {
...
static bool init_mp_psi(enum vmpressure_level level) {
    ...
    vmpressure_hinfo[level].handler = mp_event_psi;
...
static void printLMKDConfigs() {
    ...
    ALOGD("Configs :      enable_userspace_lmk : %5s", enable_userspace_lmk ? "true" : "false");
    ...
    ALOGD("Configs :           kill_timeout_ms : %5lu,           low_ram_device : %5s,"
          "              per_app_memcg : %5s", kill_timeout_ms,
    ...
    ALOGD("Configs :        wmark_boost_factor : %5d,            cache_percent :  %.2f",
